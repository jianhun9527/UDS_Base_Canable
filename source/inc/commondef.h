 ...
 * @LastEditors  : jianhun
 * @LastEditTime : 2022-05-10 21:20:45
 * @FilePath     : \UDS_Base_CANable\source\inc\commondef.h
...
#define MEM_B(x_add)        (*((uint8_t*)(x_add)))
#define MEM_W(x_add)        (*((uint16_t*)(x_add)))
...
/*lint -e545 */ ((void*)(&(((type*)0)->field))) /*lint +e545 */

// Convert two bytes to a Word according to LSB format
#define FLIPW(ray)          ((((uint16_t)(ray)[0])<<8)|((uint16_t)(ray)[1]))

// Convert a Word to two bytes according to LSB format
#define FLOPW(ray,val) \
                do{(ray)[0]= ((val)>>8);(ray)[1]=((val)&0xFF);}while(0)

// Get the high and low bytes of a word
#define WORD_LO(var)        ((uint8_t)((uint16_t)(var)&0xff))
#define WORD_HI(var)        ((uint8_t)((uint16_t)(var)>>8))
...
#define ARRAY_SIZE(a)       (((uint16_t)sizeof(a))/((uint16_t)sizeof(a[0])))
...
#define _LOG_DOUT(...)              _LOG_INF(__VA_ARGS__)
...
#define _LOG_DOUT(...)              do{SET_WIN_WHITE;printf(__VA_ARGS__);SET_WIN_WHITE;}while(0)
...
#define LOG_DOUT(...)               _LOG_DOUT(__VA_ARGS__)
#else
#define LOG_DBG(...)
#define LOG_INF(...)
#define LOG_WAR(...)
